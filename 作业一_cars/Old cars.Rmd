...
hist(logbaozhilv,col="yellow",xlab="Log保值率",ylab="频数", main="Log保值率")
```
```{r analysis preset}
...
func<-function(a,cxname){
  ...
  for(i in c(1:nrow(chexing))){
    ...
    else if(chexing[i,"中型车"]==1)
      chexing[i,ncol(chexing)]="中型车"
    else
  ...
  dataset1
  plotname<-paste(cxname)
  graph<-boxplot(value ~ group,main=plotname,col="yellow",ylab="Log保值率",dataset1)
